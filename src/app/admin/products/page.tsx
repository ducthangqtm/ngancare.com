        ...
        <div className="block md:hidden space-y-3.5 mb-6">
          {isLoading ? (
            ...
            products.map((item) => (
              ...
              >
                {/* Header card: Tên sản phẩm + Nút trạng thái */}
                <div className="flex items-start justify-between gap-2 border-b border-gray-100 pb-2.5">
                  <div>
                    <h3 className="text-sm font-extrabold text-charcoal-900 leading-snug">{item.name}</h3>
                ...
                {/* Giá bán và Link Affiliate */}
                <div className="bg-cream-50/80 p-3 rounded-xl border border-gold-100/70 text-xs space-y-2.5">
                  <div className="flex items-baseline justify-between">
                    <span className="text-gray-500 font-medium text-xs">Đơn giá niêm yết:</span>
                    <span className="text-base font-black text-gold-600">
                      {item.price ? item.price.toLocaleString('vi-VN') + 'đ' : '—'}
                    </span>
                  </div>

                  <div className="pt-1.5 border-t border-gold-200/50 flex items-center justify-between gap-2">
                    <span className="text-gray-500 text-xs font-medium">Link tiếp thị:</span>
                    {item.affiliate_url ? (
                      <a
                        href={item.affiliate_url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-bold bg-amber-100 hover:bg-amber-200 text-amber-900 transition-colors truncate max-w-[200px]"
                        title={item.affiliate_url}
                      >
                        <ExternalLink className="w-3.5 h-3.5 flex-shrink-0 text-amber-700" />
                        <span className="truncate">Mở Link Sàn TMĐT</span>
                      </a>
                    ) : (
                      <span className="text-xs text-gray-500 italic bg-gray-100 px-2 py-0.5 rounded">Tư vấn qua Zalo</span>
                    )}
                  </div>
                </div>

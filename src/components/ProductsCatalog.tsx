...
import { MessageCircle, Check, Sparkles, ShoppingCart, ExternalLink } from 'lucide-react';
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (data.success && Array.isArray(data.data) && data.data.length > 0) {
          if (isMounted) setProducts(data.data);
        } else {
          if (isMounted) setProducts(INITIAL_SERVICES.filter((s) => s.category === 'san_pham'));
        }
      } catch (e) {
        if (isMounted) setProducts(INITIAL_SERVICES.filter((s) => s.category === 'san_pham'));
      } finally {
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[1, 2, 3, 4].map((idx) => (
              <div
                key={`skeleton-${idx}`}
                className="bg-white rounded-3xl border border-gold-200 overflow-hidden shadow-sm flex flex-col justify-between"
              >
                <div>
                  {/* Skeleton Image */}
                  <div className="aspect-square w-full bg-stone-200 animate-pulse" />

                  {/* Skeleton Details */}
                  <div className="p-5">
                    <div className="space-y-2">
                      <div className="h-4 bg-stone-200 rounded-md w-full animate-pulse" />
                      <div className="h-4 bg-stone-200 rounded-md w-3/4 animate-pulse" />
                    </div>

                    <div className="mt-3 mb-4">
                      <div className="h-6 bg-stone-200 rounded-md w-28 animate-pulse" />
                    </div>

                    <div className="space-y-1.5 mb-4">
                      <div className="h-3 bg-stone-200 rounded w-full animate-pulse" />
                      <div className="h-3 bg-stone-200 rounded w-5/6 animate-pulse" />
                      <div className="h-3 bg-stone-200 rounded w-4/6 animate-pulse" />
                    </div>

                    <div className="space-y-2">
                      <div className="h-3 bg-stone-200 rounded w-4/5 animate-pulse" />
                      <div className="h-3 bg-stone-200 rounded w-3/4 animate-pulse" />
                    </div>
                  </div>
                </div>
                ...
                <div className="p-5 pt-0 space-y-2">
                  <div className="h-10 bg-stone-200 rounded-xl w-full animate-pulse" />
                  <div className="h-8 bg-stone-200 rounded-xl w-full animate-pulse" />
                </div>
              </div>
            ))}
          </div>
